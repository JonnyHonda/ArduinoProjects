...
bool WH1080::crcCheck(void) {
  if (this->buffer_idx >= 10 && this->_crc8(this->_byte_buffer, 9) == this->_byte_buffer[9]) {
    Serial.println("");
...
void WH1080::decode(void) {
  /*
  Put the currect raw value in an array
  */
  for (int l = 0; l < BUFFER_SIZE; l++) {
    rawData[l] = _byte_buffer[l];
  }
  ...
  */
  this->_deviceID = _byte_buffer[0];
  this->_deviceID = this->_deviceID << 4;

  // this->now need to add the high 4 bits of [1] (c)
  this->_deviceID = this->_deviceID + (_byte_buffer[1] >> 4);
...
void WH1080::test (void) {
  ...
  Data Sample
  Byte 0123456789
  Nibbleab cd ef gh ij kl mn op qr st
  ...
  st: checksum
  */
  byte test_data[] = {0xa1, 0x62, 0x0e, 0x5d, 0x02, 0x04, 0x00, 0x4e, 0x06, 0x86};
  for (int l = 0; l < BUFFER_SIZE; l++) {
    _byte_buffer[l] = test_data[l];
  }
  this->decode();
...
void WH1080::getRawData(byte (&data)[BUFFER_SIZE]) {
  for (int l = 0; l < BUFFER_SIZE; l++) {
    data[l] = rawData[l];
  }
}
